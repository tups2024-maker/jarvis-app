...
function pruneLegacyNav(){document.querySelectorAll('[data-go="jobs"],[data-go="market"],[data-go="business"],#jobs,#market,#business').forEach(el=>el.style.display='none')}
function simplifyPeople(){const p=document.getElementById('people');if(!p)return;const h=p.querySelector('h2');if(h&&/求人|採用/.test(h.textContent))h.textContent='ドライバー管理';p.querySelectorAll('.metrics,.linkbtn').forEach(el=>{const t=el.textContent||'';if(/求人|応募|採用|SNS/.test(t))el.style.display='none'});p.querySelectorAll('.notice').forEach(el=>{if(/求人|応募|採用|SNS/.test(el.textContent||''))el.textContent='ドライバー台帳・稼働状況の管理に使用します。'})}
function cleanCopy(){const root=document.getElementById('aiOffice');if(!root)return;root.querySelectorAll('p,.ups-note').forEach(el=>{let t=el.innerHTML;t=t.replace('配送・売上・人材・営業・経理・AI事業まで、なんでも聞いてください。','配送・売上・ドライバー・経理を中心にサポートします。');t=t.replace('配送・売上・ドライバー・営業・経理まで、なんでも聞いてください。','配送・売上・ドライバー・経理を中心にサポートします。');t=t.replace('経理・配送・営業・採用・分析などの専門機能','経理・配送・ドライバー管理・分析などの専門機能');t=t.replace('経理・配送・営業・分析などの専門機能','経理・配送・ドライバー管理・分析などの専門機能');el.innerHTML=t});const footer=root.querySelector('.ups-footer span:first-child');if(footer)footer.textContent=`UP'S AI // ${VERSION}`}
