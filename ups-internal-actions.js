(()=>{
  ...
  function shiftValue(m){
    ...
    if(/一宮/.test(m))return'一宮';if(/静岡|駿河/.test(m))return'静岡';if(/富士/.test(m))return'富士';if(/野洲|滋賀/.test(m))return'野洲';if(/中村|名古屋/.test(m))return'名古屋';if(/ロケット|\bR\b/i.test(m))return'R';if(/三島|Amazon|アマゾン|出勤|通常/.test(m))return'〇';return'';
  }
  ...
  async function prepareShiftChange(m){
    if(!/(シフト|出勤|休み|欠車|研修|三島|一宮|静岡|駿河|富士|野洲|中村|名古屋|ロケット|Amazon|アマゾン|お酒|製麺)/.test(m)||!/(変更|修正|追加|登録|反映|にして|休み|欠車|出勤)/.test(m))return null;
    const dm=m.match(/(?:(\d{1,2})月)?(\d{1,2})日/),value=shiftValue(m);if(!dm||!value)return null;
    const res=await originalFetch(`${SHIFT_API}/shift?t=${Date.now()}`,{cache:'no-store'}),json=await res.json();if(!res.ok||!json.success||!json.data?.values)throw new Error('シフト正本を取得できません');
    const data=json.data,values=data.values||[],sheetMonth=String(data.sheetName||'').match(/(20\d{2})年(\d{1,2})月/),requestedMonth=dm[1]?Number(dm[1]):Number(sheetMonth?.[2]);
    ...
    const msg=clean(m),candidates=values.map((row,i)=>({name:String(row?.[2]||'').trim(),row:i})).filter(x=>x.name).filter(x=>{const full=clean(x.name),surname=clean(x.name.split(/[ 　]/)[0]);return msg.includes(full)||msg.includes(surname)});
    const unique=[...new Map(candidates.map(x=>[clean(x.name),x])).values()];if(unique.length!==1)return{handled:true,approvalRequired:false,reply:unique.length?'同じ名字の方がいるため、フルネームで指定してください。':'対象のドライバー名をシフト正本から確認できませんでした。フルネームで指定してください。'};
    const head=values[0]||[];let best={start:-1,len:0};for(let c=3;c<head.length;c++){if(Number(head[c])!==1)continue;let len=1;while(c+len<head.length&&Number(head[c+len])===len+1)len++;if(len>best.len)best={start:c,len}}
    const day=Number(dm[2]);if(best.start<0||day<1||day>best.len)return{handled:true,approvalRequired:false,reply:'指定日の列をシフト正本で確認できませんでした。'};
    const target=unique[0],column=best.start+day-1,range=`${colLetter(column+1)}${target.row+1}`,pending={range,value,name:target.name,month:requestedMonth,day,area:screenArea(m),createdAt:Date.now()};sessionStorage.setItem(SHIFT_PENDING_KEY,JSON.stringify(pending));
    return{handled:true,approvalRequired:true,reply:`変更内容を確認します。${requestedMonth}月${day}日、${target.name}さんのシフトを「${value}」に変更します。よろしければ「この内容で確定」、やめる場合は「キャンセル」と話してください。`};
  }
  async function confirmShift(m){
    ...
    const res=await originalFetch(`${SHIFT_API}/shift/save`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({range:p.range,values:[[p.value]]})}),json=await res.json();if(!res.ok||json.success===false)throw new Error(json.error||'保存失敗');sessionStorage.removeItem(SHIFT_PENDING_KEY);
    if(window.UPS_SHIFT_SITES){await window.UPS_SHIFT_SITES.load();window.UPS_SHIFT_SITES.show(p.area||'all')}if(typeof window.upsSkillsRefresh==='function')await window.upsSkillsRefresh(true).catch(()=>null);
    return{handled:true,approvalRequired:false,reply:`${p.month}月${p.day}日、${p.name}さんのシフトを「${p.value}」へ保存しました。シフトと配送管理の最新データを再取得し、更新後の画面を表示しました。`};
  }
