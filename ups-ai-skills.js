(()=>{
  ...
  function compact(label,d){if(!d)return `${label}:未取得`;try{const s=JSON.stringify(d);return `${label}:${s.length>6500?s.slice(0,6500)+'…':s}`}catch{return `${label}:取得済み（整形不可）`}}
  ...
  async function refresh(force=false){if(!force&&Date.now()-lastRefresh<60000)return {finance,shift,delivery};lastRefresh=Date.now();const [f,s,d]=await Promise.all([getJson(`./finance-status.json?t=${Date.now()}`),getJson(`${API}/shift?t=${Date.now()}`),getJson(`${API}/delivery?t=${Date.now()}`)]);if(f)finance=f;if(s)shift=s;if(d)delivery=d;const detail={version:VERSION,finance,shift,delivery};window.dispatchEvent(new CustomEvent('ups-skills-context',{detail}));return detail}
