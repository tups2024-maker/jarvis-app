(()=>{
  ...
  async function chat(message,{voice=false}={}){
    ...
    try{
      ...
      const reply=j.reply||'回答を取得できませんでした';add(reply,'ai');if(j.responseId){previousResponseId=j.responseId;localStorage.setItem(KEY_ID,previousResponseId)}emit('ups-chat-reply',{message,reply,responseId:j.responseId||null,voice});setStatus('UP’s AI READY');if(voice)await speak(reply);return reply;
    }catch(e){const reply='通信に失敗しました。もう一度お試しください。';add(reply,'ai');setStatus('ERROR');emit('ups-chat-error',{message,error:String(e)});return null}
  ...
  function enhanceUi(){restore();const input=document.getElementById('chat'),send=document.getElementById('send');if(input){input.placeholder='アップズ君にメッセージ';input.onkeydown=e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();const t=input.value;input.value='';chat(t)}}}if(send)send.onclick=()=>{const t=input?.value||'';if(input)input.value='';chat(t)};const card=logEl()?.parentElement;if(card&&!document.getElementById('upsNewChat')){const b=document.createElement('button');b.id='upsNewChat';b.type='button';b.textContent='＋ 新しい会話';b.style.cssText='margin:8px 0 0;padding:8px 12px;border:1px solid #286b7b;border-radius:999px;background:#051d29;color:#aef8ff;font-weight:800;cursor:pointer';b.onclick=resetChat;const h=card.querySelector('h2');if(h)h.insertAdjacentElement('afterend',b);else card.prepend(b)}setStatus(previousResponseId?'CONTEXT READY':'UP’s AI READY')}
