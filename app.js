...
async function loadJarvisData(){
 try{
   ...
   const locs=['静岡','三島','一宮','中村区','野洲','富士・駿河'];
   if(routeRows.length>=4){
     const groups=[
       ['静岡','三島'],
       ['一宮','中村区'],
       ['野洲'],
   ...
   const absenceBig=document.querySelector('#page-absence .big-number');
   if(absenceBig)absenceBig.textContent=`${d.absence} 件`;
   const ai=document.querySelector('#page-dashboard .ai-summary p');
   if(ai){
      const shortage=d.shortage;
      ai.textContent=shortage>0
        ? `本日は稼働 ${d.active}/${d.requested} 台。拠点別の不足は合計 ${shortage} 台、×印の欠車は ${d.absence} 件です。欠車・不足拠点を優先確認してください。`
        : `本日は稼働 ${d.active}/${d.requested} 台。現時点で不足はありません。`;
   }

   const locCards=document.querySelectorAll('#page-locations .location');
 ...
 }catch(e){console.error('JARVIS data load failed',e)}
}
loadJarvisData();
